...
3. **Writes the content.** For each story Claude writes a plain English title and
   ...
   can edit without touching code. The model is always Claude Opus
   (`claude-opus-5`); this is fixed in the code and cannot be changed from the
   `.env` file, because Iker asked on 2026-09-22 that DeViDa never uses Fable.
   Each story also keeps the date it was
